...
ORIG_TARBALL="${PACKAGE_NAME}_${jujutsu_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${jujutsu_VERSION}"

echo "Creating Debian/Ubuntu source packages for jujutsu ${jujutsu_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/jj-vcs/jj/archive/refs/tags/v${jujutsu_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${jujutsu_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as jj-<version>/ so normalize to our BUILD_DIR
    if [ ! -d "$BUILD_DIR" ]; then
        mv jj-* "$BUILD_DIR"
    fi
...
jujutsu (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${jujutsu_VERSION}.
