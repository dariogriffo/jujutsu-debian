...
# Function to map Ubuntu architecture to jujutsu release name
get_jujutsu_release() {
    local arch=$1
    case "$arch" in
        "amd64")
...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$jujutsu_VERSION-${BUILD_VERSION}+${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "jujutsu-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "jujutsu-ubuntu-$dist-$build_arch")"
